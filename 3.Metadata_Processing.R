...
process_treatment_data <- function(treatment_df) {
  treatment_df %>%group_by(TreatID) %>% mutate(Type = ifelse(n() == 1, "Unique", "Multiple")) %>%
    ungroup() %>%distinct(TreatID, .keep_all = TRUE) %>%mutate( CAZ_HH = sub("^(CAZ-[^-]+)-.*$", "\\1", PID))}
# ===============================
# 3. READ & FILTER CAZ METADATA
# ===============================
read_caz_metadata <- function(metadata_file) {read.csv(metadata_file, check.names = FALSE) %>% filter(!TP %in% c("NC", "EC"))}
# ===============================
# 4. MERGE METADATA + TREATMENT
# ===============================
merge_metadata_treatment <- function(meta_df, treatment_df) {inner_join(meta_df, treatment_df, by = "CAZ_HH")}
# ===============================
# 5. MAIN PIPELINE
# ===============================
# Treatment data
Treatment_Data <- read_and_merge_treatment("Treatmentcode_A.csv","Treatmentcode_B.csv")
...
Merged_Meta <- merge_metadata_treatment(Caz_Meta_Data,Treatment_Data_P)
